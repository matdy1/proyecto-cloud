...
def visualizar_topologia(vms, topologia):
    ...
    if topologia.lower() == "anillo":
        for i in range(len(vms)):
            G.add_edge(vms[i]['nombre'], vms[(i+1) % len(vms)]['nombre'])
...
def configurar_vms(topologia):
    ...
    for i in range(1, num_vms + 1):
        ...
        # Preparar comando para crear VLAN
        vlan_tag = i * 100
        ip_base = f"192.168.{i}"
    ...
    # Configurar comunicación entre VLANs para topología de anillo
    if topologia.lower() == "anillo":
        for i in range(num_vms):
            vlan1 = vms[i]["vlan_tag"]
            vlan2 = vms[(i + 1) % num_vms]["vlan_tag"]  # Esto crea el anillo
    ...
    for vm in vms:
        ...
        print(f"Comando a ejecutar: {create_vm_command}")  # Aquí se imprime el comando completo
