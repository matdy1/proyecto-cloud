...
def create_linear_topology(topology_name, num_nodes, image_id=DEFAULT_IMAGE_ID, flavor_id=DEFAULT_FLAVOR_ID):
    ...
    try:
        ...
        # Step 5: Create Linear Topology Instances
            print("\nPorts created:")
            for i, port in enumerate(topology_resources['ports'], 1):
                print(f"Port {i}: {port}")

            for node_num in range(1, num_nodes + 1):
                instance_name = f"{topology_name}node{node_num}"
                
                if node_num == 1 or node_num == num_nodes:
                    # Primera y última instancia tienen un solo puerto
                    instance_ports = [topology_resources['ports'][node_num - 1]]
                else:
                    # Instancias intermedias tienen dos puertos
                    instance_ports = [
                        topology_resources['ports'][node_num - 2], 
                        topology_resources['ports'][node_num - 1]
                    ]
